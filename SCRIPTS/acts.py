...
##||| ACTS.PY TITANIUM
##||| Change list:
##||| * Added additional functions to "Block" and "Block Release" for bow cancelling.
##||| * 
##||| * 
##||| * 
##||| * 
...
def ExecTravelBookNP():
	...
	if GameText.MapList.has_key(string.upper(Bladex.GetCurrentMap())) and (not Menu.MENU_PREACTIVATED):
		...
		pepote = Menu.Desc1
		Menu.Desc1={"Name":"TopMenuTravelBook",
...
def InitBindings (PlayerName):
	...
	if netgame.GetNetState() == 0:
		...
		SelectPreviousEnemyFunc=DefaultSelectionData.SelectPreviousEnemy
...
def TranslateBoth(line):
	parts=string.split(line, " / ")
	return MenuText.GetMenuText(parts[0]) + " / " + MenuText.GetMenuText(parts[1])
...
ConfigurableActions =[
			...
			# Gamepad stuff
			(TranslateBoth("Turn Around / Special 1"),      "Turn Around",               []),
			(TranslateBoth("Left & Right / Special 2"),      "UseAndLeftRight",               []),
			(TranslateBoth("Sneak / Lock On Enemy"),      "SneakOrSelectPreviousEnemy",               [("Release", "SneakOrSelectPreviousEnemyRelease")]),
			(TranslateBoth("Jump / Select Object"),      "JumpAndSelectObj",               []),
...
def ClearConfigurableActions():
	ClearConfigurableControls()
